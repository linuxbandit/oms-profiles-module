...
// route middleware to make sure a user has permission to see the page
function canSeePage(req, res, next) {

    //first take the page permission (see format on wiki)

    //Then check the member permission (see format on wiki)
    var groups = req.user.o.reduce(function(out, arr, index) {
      var slice = arr.split(":");
      out[slice[0]] = slice[1];
      return out;
    }, {});

    console.log(req.user.o);
    console.log(req.bodycode);
    console.log(groups);

    var memberType = groups[req.bodycode]
    console.log(memberType);

    if ( memberType === undefined || memberType.indexOf('board') === -1 ) {
      console.log ("user unauthorised to this page");
      return res.redirect('/unauthorised'); //TODO: just redirect and flash an error message or something
    }else{
       console.log ("user is k");
       return next();
    }
}


function isBoardMember(req, res, next){

  //check if the user logged in is a board member of the page he is checking
  var groups = req.user.o.reduce(function(out, arr, index) {
    var slice = arr.split(":");
    out[slice[0]] = slice[1];
    return out;
  ...
  passport.authenticate('ldapauth', 
      ...
      function(err, user, info) {
          ...
          if (req.body.remember_me) { 
            tokenManager.issueToken(req.body.uid, function(err, token) {
              if (err) { return next(err); }
              console.log("RM token issued: "+token);
...
function getMemberships(req, res, next){
    ...
    restClient.get(options, function(resterr, restreq, restres, restobj) {
      ...
      console.log(resterr);
      console.log(config.rest.core.url);
      //console.log('\n %j \n', restobj); 
...
function renderHome(req, res) {
  var activeHome = true;
  res.render('home', {layout: 'front-page', title: 'Welcome to MyAEGEE'});
};
...
function renderUnauth(req, res){
  res.render('error', {message: 'user unauthorised (not board member?)'});
}
...
router.get('/unauthorised', renderUnauth );

// route with parameters (http://localhost:8801/hello/:name)
router.get('/user/:uid', isLoggedIn, canSeePage, getMember, getMemberships, renderProfile );
...
router.get('/applicants/:bodycode', isLoggedIn, isBoardMember, getApplicationsToBody, renderApplicationsToBody );
